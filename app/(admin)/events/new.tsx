  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        price: isFree ? 0 : parseFloat(price.replace(/[^0-9.]/g, '')) || 0,
        is_free: isFree,
      ...
      Alert.alert('Éxito', 'Evento creado', [
        { text: 'Ver evento', onPress: () => (router as any).replace(`/(admin)/events/${data.id}`) },
        { text: 'Lista', onPress: () => (router as any).replace('/(admin)/events') },
      ]);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        {errors.eventDate ? <Text style={styles.error}>{errors.eventDate}</Text> : null}
        ...
        <View style={styles.categoryRow}>
          {CATEGORIES.map((cat) => (
            <Pressable
              ...
              style={[
                styles.categoryBtn,
                { backgroundColor: category === cat.value ? c.primary : c.card },
              ]}
            >
              <Text
                style={[
        ...
        />
        {errors.maxCapacity ? <Text style={styles.error}>{errors.maxCapacity}</Text> : null}
        ...
        >
          {saving ? (
            ...
            <Text style={[styles.saveBtnText, { color: c.primaryForeground }]}>Crear evento</Text>
          )}
        </Pressable>
