...
export default function AdminRestaurantsIndexScreen() {
  ...
  return (
    ...
    >
      ...
      {list.map((x) => (
        <Pressable
          key={x.id}
          onPress={() => (router as any).push(`/(admin)/restaurants/${x.id}`)}
      ...
      {list.length === 0 && (
        <Text style={[styles.empty, { color: c.mutedForeground }]}>No hay restaurantes</Text>
      )}
