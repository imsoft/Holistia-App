...
export default function AdminShopsIndexScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      console.error('Shops load error:', e);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    ...
    >
      ...
      {list.map((x) => (
        <Pressable
          key={x.id}
          onPress={() => (router as any).push(`/(admin)/shops/${x.id}`)}
      ...
      {list.length === 0 && (
        <Text style={[styles.empty, { color: c.mutedForeground }]}>No hay comercios</Text>
      )}
