  ...
  return (
    ...
    >
      <View style={[styles.card, { backgroundColor: c.card }]}>
        <Text style={[styles.label, { color: c.mutedForeground }]}>Total tickets</Text>
      ...
      {list.map((x) => (
        <View key={x.id} style={[styles.row, { backgroundColor: c.card }]}>
          <Text style={[styles.rowTitle, { color: c.foreground }]} numberOfLines={2}>
            {x.subject ?? 'Sin asunto'}
          </Text>
          <Text style={[styles.rowMeta, { color: c.mutedForeground }]}>
            {x.status ?? '—'} · {x.priority ?? '—'} · {x.created_at ? new Date(x.created_at).toLocaleDateString() : '—'}
          </Text>
        </View>
      ))}
      {list.length === 0 && (
        <Text style={[styles.empty, { color: c.mutedForeground }]}>No hay tickets</Text>
      )}
