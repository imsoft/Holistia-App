...
export default function EditChallengeScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setDurationDays(String(data.duration_days ?? 7));
      setDifficultyLevel((data.difficulty_level as DifficultyLevel) || 'beginner');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Éxito', 'Reto actualizado.', [{ text: 'OK', onPress: () => router.back() }]);
    } catch (err: any) {
  ...
  if (notFound) {
    return (
      <View style={[styles.center, { backgroundColor: c.background }]}>
        <Text style={[styles.notFound, { color: c.mutedForeground }]}>Reto no encontrado</Text>
